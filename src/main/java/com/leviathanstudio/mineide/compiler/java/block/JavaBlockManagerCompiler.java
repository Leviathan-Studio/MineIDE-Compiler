...
public abstract class JavaBlockManagerCompiler
{
    ...
    public JavaBlockManagerCompiler compile() throws IOException
    {
        ...
        this.registerBlocksMethod = MethodSpec.methodBuilder("registerBlock").addModifiers(Modifier.PRIVATE, Modifier.STATIC);
        this.registerBlocks();
        
        this.renderBlocksMethod = MethodSpec.methodBuilder("renderBlock").addModifiers(Modifier.PRIVATE, Modifier.STATIC);
        this.setRenderBlocks();
        
        TypeSpec blocksManagerBuilder = TypeSpec.classBuilder(this.getBlocksManagerClass().simpleName()).addField(this.blockSuperclass, "blockTest", Modifier.PUBLIC, Modifier.STATIC).addModifiers(Modifier.PUBLIC).addMethod(initMethod.addStatement("$N()", "initBlock").addStatement("$N()", "registerBlock").addStatement("$N()", "renderBlock").build()).addMethod(this.initBlocksMethod.build()).addMethod(this.registerBlocksMethod.build()).addMethod(this.renderBlocksMethod.build()).build();
        
        this.setBlockManagerJavaFile(JavaFile.builder(this.getBlocksManagerClass().packageName(), blocksManagerBuilder).build());
        
        this.getBlockManagerJavaFile().writeTo(Util.FORGE_SRC_JAVA_DIR);

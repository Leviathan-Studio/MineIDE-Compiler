...
public abstract class JavaBlockCompiler extends JavaCompilerInformation
{
    ...
    public JavaBlockCompiler compile() throws IOException
    {
        ...
        this.setBlockBuilder(TypeSpec.classBuilder(this.getBlockClass()).superclass(this.getSuperClass())
                .addModifiers(Modifier.PUBLIC).addMethod(this.getBlockConstructor()).build());

        this.setBlockClassJavaFile(
                JavaFile.builder(this.getBlockClass().packageName(), this.getBlockBuilder()).build());

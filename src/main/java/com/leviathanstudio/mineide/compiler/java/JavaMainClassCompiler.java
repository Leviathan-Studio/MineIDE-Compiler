...
public class JavaMainClassCompiler
{
    ...
    public ClassName   preInitEventClass  = ClassName.get(this.commonPackageForge + ".event",
            "FMLPreInitializationEvent");
    public ClassName   initEventClass     = ClassName.get(this.commonPackageForge + ".event", "FMLInitializationEvent");
    public ClassName   postInitEventClass = ClassName.get(this.commonPackageForge + ".event",
    ...
    public void compile() throws IOException
    {
        ...
        AnnotationSpec instanceAnnotation = AnnotationSpec.builder(instanceClass).addMember("value", "$S", "MODID_TEST")
                .build();

        ClassName modClass = ClassName.get(this.commonPackageForge, "Mod");
        AnnotationSpec modAnnotation = AnnotationSpec.builder(modClass).addMember("modid", "$S", "MODID_TEST")
                .addMember("name", "$S", "NAME_TEST").addMember("version", "$S", "VERSION_TEST").build();

        ClassName sidedProxyClass = ClassName.get(this.commonPackageForge, "SidedProxy");
        AnnotationSpec sidedProxyAnnotation = AnnotationSpec.builder(sidedProxyClass)
                .addMember("clientSide", "$S", clientProxyClass).addMember("serverSide", "$S", serverProxyClass)
                .build();

        FieldSpec instanceField = FieldSpec.builder(classMain, "instance", Modifier.PUBLIC, Modifier.STATIC)
                .addAnnotation(instanceAnnotation).build();

        FieldSpec proxyField = FieldSpec.builder(commonProxyClass, "proxy", Modifier.PUBLIC, Modifier.STATIC)
                .addAnnotation(sidedProxyAnnotation).build();

        FieldSpec loggerField = FieldSpec.builder(loggerClass, "logger", Modifier.PUBLIC, Modifier.STATIC).build();

        TypeSpec mainClass = TypeSpec.classBuilder(classMain).addAnnotation(modAnnotation).addModifiers(Modifier.PUBLIC)
                .addField(instanceField).addField(proxyField).addField(loggerField).addMethod(this.getPreInitMethod())
                .addMethod(this.getInitMethod()).addMethod(this.getPostInitMethod()).build();

...
public abstract class JavaProxiesCompiler
{
    ...
    public JavaProxiesCompiler compile() throws IOException
    {
        ...
        this.getClientProxyJavaFile().writeTo(Util.FORGE_SRC_JAVA_DIR);
        this.getCommonProxyJavaFile().writeTo(Util.FORGE_SRC_JAVA_DIR);
        this.getServerProxyJavaFile().writeTo(Util.FORGE_SRC_JAVA_DIR);
        return this;
    }
    
    private void initClientProxy() throws IOException
    {
        TypeSpec clientProxyBuilder = TypeSpec.classBuilder(this.clientProxyClass).superclass(this.commonProxyClass).addModifiers(Modifier.PUBLIC).addMethod(this.preInitMethod.addAnnotation(Override.class).addStatement("super.preInit(configFile)").build()).addMethod(this.initMethod.addAnnotation(Override.class).addStatement("super.init()").build()).build();
    ...
    private void initCommonProxy() throws IOException
    {
        ...
        TypeSpec commonProxyBuilder = TypeSpec.classBuilder(this.commonProxyClass).addModifiers(Modifier.PUBLIC).addMethod(this.preInitMethod.build()).addMethod(this.initMethod.build()).build();
    ...
    {
        TypeSpec serverProxyBuilder = TypeSpec.classBuilder(this.serverProxyClass).superclass(this.commonProxyClass).addModifiers(Modifier.PUBLIC).addMethod(this.preInitMethod.build()).addMethod(this.initMethod.build()).build();

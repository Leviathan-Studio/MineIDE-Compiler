...
public class MineIDECompiler
{
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        JavaBlockCompiler blockTest2 = new JavaBlockCompiler()
        {
            ...
            {
                this.setBlockConstructor(MethodSpec.constructorBuilder().addModifiers(Modifier.PUBLIC)
                        .addCode(this.getConstructorSpec().build()).build());
            }
            ...
            public void initializeBlockClass()
            {
                ...
                this.getConstructorSpecList()
                        .add(CodeBlock.builder().addStatement("super($T.$N)", materialClass, "WOOD").build());
        ...
        JavaGameRegistry gameRegistry = new JavaGameRegistry()
        {
            ...
            {
                this.getGameRegistryList()
                        .add(CodeBlock.builder().addStatement("$T.registerBlock(" + "$N" + ", " + "\"$L\"" + ")",
                                this.gameRegistryClass, "blockTest", "BlockTest").build());
        ...
        JavaBlockManagerCompiler blocksManagerCompiler = new JavaBlockManagerCompiler()
        {
            ...
            {
                this.setBlocksManagerClass(
                        ClassName.get(mainClassCompiler.getMainClassPackage() + ".block", "BlocksManager"));
            }

            @Override
            public void initializeBlocks()
            {
                this.initBlocksMethod.addCode(CodeBlock.builder()
                        .addStatement("$N = new $L()", "blockTest", blockTest2.getBlockClassName()).build());
            }
        ...
        mainClassCompiler.setPreInitMethod(MethodSpec.methodBuilder("preInit").addModifiers(Modifier.PUBLIC)
                .addAnnotation(mainClassCompiler.eventHandlerClass)
                .addParameter(mainClassCompiler.preInitEventClass, "event").addStatement("logger = event.getModLog()")
                .addStatement("proxy.preInit(event.getSuggestedConfigurationFile())")
                .addStatement("$T.init()", blocksManagerCompiler.getBlocksManagerClass()).build());
        mainClassCompiler.setInitMethod(MethodSpec.methodBuilder("init").addModifiers(Modifier.PUBLIC)
                .addAnnotation(mainClassCompiler.eventHandlerClass)
                .addParameter(mainClassCompiler.initEventClass, "event").addStatement("proxy.init()").build());
        mainClassCompiler.setPostInitMethod(MethodSpec.methodBuilder("postInit").addModifiers(Modifier.PUBLIC)
                .addAnnotation(mainClassCompiler.eventHandlerClass)
                .addParameter(mainClassCompiler.postInitEventClass, "event").build());
